...
contract AZINA_FTM is ReentrancyGuard{
    ...
    //Spookyswap Router - Required to liquidate collateral
    ...
    uint256 public s_directDeposit; // holds FTM that is sent directly to the contract. To allow recovery
    ...
    function loanRequest( uint256 _loanAmount, uint256 _duration, uint256 _paymentMethodId) external  payable returns (uint256 _loanId){
        ...
        require(VerificationInterface.checkVerificationStatus(msg.sender), "Not Authorised To Use Azina services");
    ...
    // liquidate through Spookyswap in mainnet
    ...
    function performUpkeep(bytes calldata performData ) external payable {
        ...
        //check if loan expired or reached liquidation threshold
    ...
    // gets the current price of FTM from chainlink oracles in USD
    ...
    // wraps FTM to WFTM
    // swaps WFTM for USDC on Spookyswap
    function liquidateCollateral(uint256 _collateralAmount, uint256 _currentPrice) internal returns(uint256 amountOut) {
     
        address[] memory path = new address[](2);
        path[0] = WFTM;
        path[1] = STABLECOIN;
        uint256 amountOutMinimum = uint256((s_liquidationThreshold *_collateralAmount*_currentPrice) / (100 * 1e18 * 10**s_priceDecimals)); //atleast liquidationThreshold % at current prices

        uint256[] memory amounts = UniswapInterface.swapExactETHForTokens{value: _collateralAmount}(amountOutMinimum, path, s_owner, block.timestamp + 30 minutes);
